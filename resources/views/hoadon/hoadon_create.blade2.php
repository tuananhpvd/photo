...
<form class="card p-3 bg-light" method="post" action="{{ route('hoadon_store')}}">
    ...
    @csrf
        ...
        <select  class="form-control" required="true" name="tenlop" id="myList" onchange="myFunction()">
            
            <option value="" name="tenlop" id="tenlop" > - Chọn lớp - </option>
            
            @foreach($lops as $lop)

            <option value="{{$lop->id}},{{$lop->siso}}" name="tenlop" id="tenlop">{{$lop->tenlop}}</option>
    ...
    <p>
        <input type="hidden" name="malop" value="" id="ml">
    </p>
...
<input class="form-control" required="true" type="number" name="sotrang" value="">
        ...
        <label class="col-form-label" for="soluong">Số lượng: </label>
                    ...
                    <div class="col-md-7">
<input class="form-control" required="true" type="text" name="soluong" value="" id="sl">
                    </div>
                </div> 
